...
const PRODUCTS = [
  { id: 'p1', type: 'reman', name: 'HD 정품 유압실린더 (Reman)', grade: 'A', price: 1200000, creditPrice: 1200000, img: '🔄' },
  { id: 'p2', type: 'reman', name: '주행 모터 어셈블리 (Reman)', grade: 'B', price: 850000, creditPrice: 850000, img: '⚙️' },
  { id: 'p3', type: 'consumable', name: '고성능 유압유 20L', price: 85000, creditPrice: 85000, img: '🛢️' },
  { id: 'p4', type: 'consumable', name: '에어필터 세트 (6EA)', price: 42000, creditPrice: 42000, img: '🌬️' },
  { id: 'p5', type: 'safety', name: '안전모 (HD현대 에디션)', price: 35000, creditPrice: 35000, img: '👷' },
];

const GROUP_BUYS = [
  { id: 'gb1', name: '고성능 유압유 20L 10명 공구', originalPrice: 85000, price: 55000, maxGroup: 10, currGroup: 7, timeLeft: '12:45:00', img: '🛢️' },
  { id: 'gb2', name: '에어필터 세트 50명 박스떼기', originalPrice: 42000, price: 29000, maxGroup: 50, currGroup: 48, timeLeft: '02:15:00', img: '🌬️' }
];

const RENTALS = [
  { id: 'r1', name: '비 오는 날 굴착기(HX300L) 하루 빌려드립니다.', price: 300000, author: '김동현 기사', site: '현장 A', time: '10분 전', img: '🚜' },
  { id: 'r2', name: '주말 2일간 미니 굴착기 단기 렌탈 구해요!', price: 450000, author: '최현우 기사', site: '현장 C', time: '1시간 전', img: '🏗️' }
];
...
export default function Market() {
  ...
  return (
    <>
      ...
      <TitleBlock title="HI-Market" subtitle="크레딧으로 정품 부품을 구매하세요" />
      ...
      <div style={{ padding: '20px 24px 0', display: 'flex', gap: 8, overflowX: 'auto' }}>
        {[
          { id: '전체', label: '전체' },
          { id: 'reman', label: '♻️ 재제조 부품' },
          { id: 'consumable', label: '🛢️ 소모품' },
          { id: 'safety', label: '👷 안전/기타' },
          { id: 'groupbuy', label: '🤝 공동구매' },
          { id: 'rental', label: '🚜 장비 렌탈' },
        ].map(f => (
          <div key={f.id}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {GROUP_BUYS.map(gb => (
              <div key={gb.id} className="hf-glass-soft" style={{ borderRadius: 16, padding: 16, display: 'flex', gap: 16 }}>
                <div style={{ width: 80, height: 80, borderRadius: 12, background: 'var(--hf-bg-deep)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 40, flexShrink: 0 }}>
                  {gb.img}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <div style={{ fontSize: 11, color: HF.bad, fontWeight: 700 }}>⏳ {gb.timeLeft} 남음</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {RENTALS.map(r => (
              <div key={r.id} className="hf-glass-soft" style={{ borderRadius: 16, padding: 16, display: 'flex', gap: 16, border: `1px solid var(--hf-divider)` }}>
                <div style={{ width: 64, height: 64, borderRadius: 12, background: 'var(--hf-bg-deep)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, flexShrink: 0 }}>
                  {r.img}
                </div>
                <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {filtered.map(p => (
              <div key={p.id} className="fleet-card" style={{ padding: 14, display: 'flex', flexDirection: 'column' }}>
                {/* 이미지 영역 (모의) */}
                <div style={{ height: 100, borderRadius: 12, background: 'var(--hf-bg-deep)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 48, marginBottom: 12 }}>
                  {p.img}
                </div>
